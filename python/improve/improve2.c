...
#define NUM_FEATURES 11 //scoreとvalueの比
#define LAST_STAGE 50
double weight[NUM_FEATURES] = {100, 718.9, -125.2, -88.2, -93.7, -12.2, -337.9, -140.0, -149.4, -659.0, -1163.2}; // weights of the features
double value = 300.0;
...
int evaluate(int turn)
{
    ...
    if (nstones > LAST_STAGE) {
        ...
        int nmoves =  generate_legal_moves(-turn, legal_moves);
        return score + nmoves * value;
    }
}
...
void selfplay()
{
    ...
    double f; // The values of the features
    double pb = -1; // P(Winner=Black) 黒が勝つ確率
    double a = 0.001; //スライドに出てくるaそのまま
    double learning_rate = 10; //学習率(どのくらいの幅で移動するか)
    ...
    if (d > 0) value += learning_rate * (1.0 - pb) * a * f; //黒が勝ったとき
    else       value -= learning_rate * pb * a * f; //白が勝ったとき
